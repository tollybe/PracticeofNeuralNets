...
class ObstacleClass(pygame.sprite.Sprite):
    ...
    ##makes Scenary Scroll up
    def update(self):
        global speed
        self.rect.centery -= speed[1]
        ## Deletes Obstacles that have scrolled off the top of screen
        if self.rect.centery < -32:
            self.kill()


##creates one screen ull of random trees + flags
def create_map():
    global obstacles
    locations = []
    for i in range(10):
...
running = True
## checks for keypresses or window close 30 times per second
while running:
    clock.tick(30)
